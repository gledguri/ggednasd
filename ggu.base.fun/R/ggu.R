...
#' Custom cbind Function for Dataframes with Different Rows
#'
#' This function combines two dataframes with different rows by adding
#' the missing rows with \code{NA} values and then performing a column bind
#' on the resulting dataframes.
#'
#' @param q_1 A dataframe. The first dataframe to be column-bound.
#' @param w_1 A dataframe. The second dataframe to be column-bound.
#'
#' @return A dataframe that is the result of column-binding \code{q_1} and \code{w_1}.
...
custom_cbind <- function(q_1, w_1) {
  ...
  # Find columns in w_1 that are not in q_1
  rows_w_1_not_in_q_1 <- setdiff(rownames(w_1), rownames(q_1))
  
  # Add missing columns to w_1 with NA values
  if (length(rows_q_1_not_in_w_1) > 0) {
    w_1[rows_q_1_not_in_w_1,] <- NA
  }
  
  # Add missing columns to q_1 with NA values
  if (length(rows_w_1_not_in_q_1) > 0) {
    q_1[rows_w_1_not_in_q_1,] <- NA
  }
